...
function savedAgendasReducer(state, { type, payload }) {
  switch (type) {
    ...
    case 'update':
      return state.map((agd) => (agd.id === payload.id ? payload : agd));
    case 'delete':
      ...
      <div className="App w-screen h-screen">
        <div className="navbar w-full h-[66px] items-center flex justify-between">
          <div className="brand items-center flex">
            ...
            <h1 className="ml-8 text-xl">LOGO</h1>
          </div>
          <div className="nav flex items-center">
            ...
            <button
              ...
              className="btn-logout flex items-center border border-slate-500 hover:bg-gray-300 rounded h-2/3 px-2 py-1.5 mr-8"
            >
              <div>Logout</div>
            </button>
          </div>
        ...
        <div className="content w-full h-[calc(100%-66px)] flex">
          <div
            className={`sidebar ${
              open ? 'w-[272px]' : 'w-0'
            } h-full border-solid border-r-[1px]`}
          >
            <div className="sidbar-nav-top w-full h-[32px] flex justify-evenly items-center bg-slate-200 overflow-hidden">
              <button
                className={`btn-workspace w-1/2 mx-1 ${
                  currentSidebar === 'SidebarWorkspace'
                    ? 'bg-white'
                    : 'bg-inherit'
                } rounded`}
                onClick={() => setCurrentSidebar('SidebarWorkspace')}
              >
                Work space
              </button>
              <button
                className={`btn-calendar w-1/2 mx-1 ${
                  currentSidebar === 'SidebarCalendar'
                    ? 'bg-white'
                    : 'bg-inherit'
                } rounded`}
                onClick={() => setCurrentSidebar('SidebarCalendar')}
              >
                Calendar
              </button>
            </div>
          ...
          <div
            className={`work-space ${open ? 'w-[calc(100%-272px)]' : 'w-full'}`}
          >
              ...
              <div className="nav-action flex items-center">
                <button
                  className={`btn-Agenda ${
                    currentTab === 'Agenda'
                      ? 'bg-blue-100 text-blue-400 hover:bg-blue-100'
                      : 'bg-inherit'
                  } hover:bg-gray-200 rounded-md px-2 py-1 border-solid border-red-500 mr-8`}
                ...
                <button
                  ...
                  onClick={() => setCurrentTab('Nextstep')}
                >
                  Next Step
                </button>
                <button
                ...
                <a
                  href="#"
                  className={`btn-PastMeetings text-xl ${
                    currentTab === 'PastMeetings'
                      ? 'bg-blue-100 text-blue-400 hover:bg-blue-100'
                      : 'bg-inherit'
                  } hover:bg-gray-200 rounded-md p-1.5 border-solid border-red-500 mr-8`}
                  onClick={() => setCurrentTab('PastMeetings')}
                >
                  <FaPaste className="" />
                </a>
                <a
                  href="#"
                  className="btn-Scratchpad p-1.5 rounded hover:bg-gray-200 mr-8"
                >
                ...
                <a
                  href="#"
                  className={`btn-Settings text-xl ${
                    currentTab === 'Settings'
                      ? 'bg-blue-100 text-blue-400 hover:bg-blue-100'
                      : 'bg-inherit'
                  } hover:bg-gray-200 rounded-md p-1 border-solid border-red-500 mr-8`}
